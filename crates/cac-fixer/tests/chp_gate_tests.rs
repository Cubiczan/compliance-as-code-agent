...
/// Fixture secret line, assembled at runtime: the compliance self-scan of
/// this test file must not flag the fixture as a real hardcoded secret,
/// while the assembled string still matches the no-hardcoded-secrets policy
/// when written to the temp repo.
fn secret_line() -> String {
    format!("let {} = \"sk-{}\";", "api_key", "abc12345678")
}

fn still_secret_line() -> String {
    format!("let {} = \"sk-{}\";", "api_key", "ZZZ99988877")
}
...
fn still_secret_proposal() -> FixProposal {
    ...
    FixProposal {
        ...
        file_path: "src/config.rs".to_string(),
        original_snippet: secret_line(),
        fixed_snippet: still_secret_line(),
...
fn r0_refusal_records_decision() {
    ...
    assert!(content.contains(secret_line().as_str()));
...
fn parity_failure_is_fatal_and_recorded() {
    ...
    let content = std::fs::read_to_string(tmp.path().join("src/config.rs")).unwrap();
    assert!(
        content.contains(secret_line().as_str()),
