...
use te_core::{
    canonical_usage, dashboard_summary, eps_impact, estimate_cost, forecast, monthly_snapshot,
...
fn main() {
    ...
    let output = match command.as_str() {
        ...
        "roi-scenario" => {
            ...
            let annual_token_cost = per_run
                .get("total_cost_usd")
                .and_then(Value::as_f64)
                .unwrap_or(0.0)
                * annual_runs
                * 1.85;
            let labor_value = input
                .get("avg_hours_saved")
                .and_then(Value::as_f64)
                .unwrap_or(0.0)
                * input
                    ...
                    .unwrap_or(150.0)
                * successful_runs;
            let revenue_value = input
                .get("avg_revenue_lift_usd")
                .and_then(Value::as_f64)
                .unwrap_or(0.0)
                * successful_runs;
            let manual_savings = input
                .get("manual_baseline_cost_usd")
                .and_then(Value::as_f64)
                .unwrap_or(0.0)
                * successful_runs;
            let annual_value = labor_value + revenue_value + manual_savings;
            let net_value = annual_value - annual_token_cost;
            let roi = if annual_token_cost > 0.0 {
                annual_value / annual_token_cost
            } else {
                0.0
            };
            let cost_per_task = if successful_runs > 0.0 {
                annual_token_cost / successful_runs
            } else {
                0.0
            };
            let payback = if net_value > 0.0 && annual_token_cost > 0.0 {
                Some((annual_token_cost / (net_value / 12.0)).round())
            } else {
                None
            };
            let rec = if roi >= 5.0 {
